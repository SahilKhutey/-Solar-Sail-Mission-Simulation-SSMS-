...
class PlotsPanel(QWidget):
    ...
    def init_ui(self):
        ...
        self.diagnostics_browser.page().setBackgroundColor(self.palette().color(self.backgroundRole()))
    ...
    def plot_data(self, history, config=None):
        ...
        fig1 = make_subplots(rows=2, cols=2, subplot_titles=("Semi-major Axis (km/AU)", "Eccentricity", "Inclination (deg)", "Altitude/Dist"))
        ...
        fig1.add_trace(go.Scatter(x=t_days, y=a_plot, name=lbl_a, line=dict(color='#00aaff')), row=1, col=1)
        fig1.add_trace(go.Scatter(x=t_days, y=e, name='Ecc', line=dict(color='#ffaa00')), row=1, col=2)
        fig1.add_trace(go.Scatter(x=t_days, y=i_deg, name='Inc', line=dict(color='#00ffaa')), row=2, col=1)
        fig1.add_trace(go.Scatter(x=t_days, y=dist_plot, name=lbl_dist, line=dict(color='#ff5555')), row=2, col=2)
        
        self._apply_theme(fig1)
        self.elements_browser.setHtml(pio.to_html(fig1, include_plotlyjs='cdn'))
        
        # --- Plot 2: Diagnostics ---
        fig2 = make_subplots(rows=2, cols=2, subplot_titles=("Specific Energy", "Angular Momentum", "Hamiltonian Error (Drift)", "Eclipse Flag"))
        
        fig2.add_trace(go.Scatter(x=t_days, y=eps, name='Energy', line=dict(color='#ff5555')), row=1, col=1)
        fig2.add_trace(go.Scatter(x=t_days, y=h, name='Ang Mom', line=dict(color='#00aaff')), row=1, col=2)
        
        # Drift (Relative to initial)
        d_eps = (eps - eps[0]) / abs(eps[0]) if abs(eps[0]) > 1e-9 else eps
        fig2.add_trace(go.Scatter(x=t_days, y=d_eps, name='Energy Drift', line=dict(color='#ffff00')), row=2, col=1)
        
        # Eclipse (Placeholder for now, need in history)
        # For now, just plot zero
        fig2.add_trace(go.Scatter(x=t_days, y=np.zeros_like(t_days), name='Eclipse', line=dict(color='#888888')), row=2, col=2)
    ...
    def _show_empty(self, browser):
        fig = go.Figure()
        fig.update_layout(title="No Data", template="plotly_dark", paper_bgcolor="#1e1e1e", plot_bgcolor="#1e1e1e")
