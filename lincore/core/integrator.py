...
def rk45_step(f, t, y, dt, tol, *args, dt_max=None):
    ...
    k6 = np.array(
        f(t + c6 * dt, y_np + dt * (a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5), *args)
    )
...
def velocity_verlet_step(f_acc, t, r, v, dt, *args):
    """
    Velocity Verlet (Symplectic for conservative forces).
    r, v: numpy arrays
    f_acc: function(t, r, v, *args) -> acceleration
    """
    r = np.array(r)
    v = np.array(v)

    # 1. Half-step velocity
    a_t = np.array(f_acc(t, r, v, *args))
    v_half = v + 0.5 * a_t * dt
    ...
    # 3. New acceleration
    a_next = np.array(f_acc(t + dt, r_next, v_half, *args))  # v_half approx or v_next?
    # Strictly, a_next depends on r_next and v_next.
    # For conservative systems a(r), it's fine.
    # For drag a(r, v), we usually use v_half or iterate.
    # Using v_half is a common approximation.
